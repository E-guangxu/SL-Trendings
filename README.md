...
默认推送的是**全站榜**（Reddit r/all + YouTube 全球热门）。想让内容跟着你关心的话题走，有三种用法。

先说一个话题会抓出什么 —— 每个话题三段：

| 段落 | 来源 | 特点 |
|---|---|---|
| 最新新闻 | Google News RSS 搜索 | 无需 key、最稳，标题自带来源 |
| Reddit 讨论（近 24 小时） | Reddit 搜索 RSS | 先搜「近一天+热度」，无结果自动退到「近一周+相关度」 |
| YouTube 最新视频 | 搜索结果页（按上传日期排序） | 带回发布时间和播放量，便于判热度 |

### 用法 1：让每天的推送固定跟这些话题（最常用）
...
保存提交即可，下一次 08:05 的推送就是这三组内容。**文件里全是注释 = 回到全站榜。**
